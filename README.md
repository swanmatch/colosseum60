...
ちょっと*一心不乱になりすぎて*組んでるときに写真を撮るのを忘れがちです。。。  
...
その逆が表(スイッチ側)となります。
ダイオードの向きはフットプリントに向かって見たときに黒い方が右側です。
...
ピン番号などが書いてあるほうが裏側です。
注意事項ですが、ProMicro側にTRRSJACKを、裏側にRESETスイッチをはんだ付けしてください。
RESETスイッチ押せないです。

ProMicroはまだはんだ付けしちゃダメです。
しろとりとの約束です。
...
各線がプレート側まで導通しているかテスターを当てて確認してください。

スプリングピンヘッダ(コンスルー)をお持ちであれば、
